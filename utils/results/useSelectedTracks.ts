...
export function useSelectedTracks({
  ...
  // Exists as a cache to making sure it doesn't re-add the same songs twice
  ...
  async function replaceSelectedAtIndex(indexToReplace: number) {
    ...
    // * If the unused songs is about to run out, get more songs from the server so never never waiting to swap songs
